...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    draw_positions_x = [30,350,670]
    
    for idx in range(len(slice_nums)):
        nd_img = np.flip((orig_vol[:, :, slice_nums[idx]]/np.max(orig_vol[:, :, slice_nums[idx]]))*0xff).T.astype(np.uint8)
        nd_pred = np.flip((pred_vol[:, :, slice_nums[idx]]/np.max(pred_vol[:, :, slice_nums[idx]]))*0xff).T.astype(np.uint8)            
        
        # This is how you create a PIL image from numpy array
        pil_i = Image.fromarray(nd_img, mode="L").convert("RGBA").resize((300,300))
        pil_pred = Image.fromarray(nd_pred, mode="L").resize((300,300))
     
        
        for x in range(300):
            for y in range(300):
                val = pil_pred.getpixel( (x,y) )                   
                if val != 0:                    
                    pil_i.putpixel( (x,y), (val,0,0,0))                
        #pil_i.paste(pil_pred,box=(0,0),pil_pred)
        #blended = Image.blend(pil_i, pil_pred, alpha=0.3)
        # Paste the PIL image into our main report image object (pimg)
        pimg.paste(pil_i, box=(draw_positions_x[idx], 280))
        #pimg.paste(pil_pred,box=(draw_positions_x[1],280))
        #pimg.paste(blended, box=(draw_positions_x[idx], 280))
...
def get_series_for_inference(path):
    ...
    filtered_paths = [dir for dir, subdirs, files in os.walk(path) if 'HCropVolume' in dir]
    
    random_study_path = np.random.choice(filtered_paths=int(len(filtered_paths) > 0) )
    
    dicoms = [pydicom.dcmread(os.path.join(random_study_path, f)) for f in os.listdir(random_study_path)]
    ...
    for s in dicoms:
        ...
        if s.SeriesDescription == 'HippoCrop':
            series_for_inference.append(s)
...
if __name__ == "__main__":
    ...
    os_command("sudo storescu 127.0.0.1 4242 -v -aec HIPPOAI /home/workspace/out/report.dcm")
